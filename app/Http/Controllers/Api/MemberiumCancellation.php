...
use App\Models\CancelLog as Model;
use App\Http\Controllers\ApiController;
use App\Http\Controllers\Transformers\MemberiumCancellation\MemberiumCancellationTransformer;

class MemberiumCancellation extends ApiController
{

	protected $transformer, $search, $page, $rows, $offset;

	function __construct(MemberiumCancellationTransformer $transformer)
	{
    ...
    public function index(Request $request)
    {
        ...
        $total = Model::where(function($query){ 
                    if ($this->search !== '') {
                        $query->orWhere('cb_receipt', 'LIKE', '%'.$this->search.'%');
